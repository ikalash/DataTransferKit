...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<GlobalOrdinal> tet_handles( num_elements );
    Teuchos::Array<GlobalOrdinal> tet_connectivity( 4 * num_elements );
    ...
    return Teuchos::rcp( new MyMesh( vertex_handles, coords, tet_handles,
                                     tet_connectivity, permutation_list ) );
}
...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_expanded_test3 )
{
    using namespace DataTransferKit;
...
// aren't in the mesh.
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_tiled_test3 )
{
    using namespace DataTransferKit;

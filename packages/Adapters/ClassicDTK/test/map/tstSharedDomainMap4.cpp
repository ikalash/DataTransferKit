...
class MeshTraits<MyMesh>
{
  public:
    ...
    static inline int verticesPerElement( const MyMesh &mesh ) { return 4; }
...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<GlobalOrdinal> quad_handles( num_elements );
    Teuchos::Array<GlobalOrdinal> quad_connectivity( 4 * num_elements );
    ...
    return Teuchos::rcp( new MyMesh( vertex_handles, coords, quad_handles,
                                     quad_connectivity, permutation_list ) );
}

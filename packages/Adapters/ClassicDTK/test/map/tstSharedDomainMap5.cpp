//---------------------------------------------------------------------------//
/*!
 * \file tstSharedDomainMap5.cpp
 * \author Stuart R. Slattery
 * \brief Shared domain map unit test 5.
 */
//---------------------------------------------------------------------------//
...
class MeshTraits<MyMesh>
{
  public:
    ...
    static inline int vertexDim( const MyMesh &mesh ) { return 2; }
    ...
    static inline int verticesPerElement( const MyMesh &mesh ) { return 3; }
                                             ...
                                             MyField>
{
  public:
              ...
              const Teuchos::ArrayRCP<double> &coords )
    {
        ...
        MyField evaluated_data( num_elements, 9 );
        for ( int n = 0; n < num_elements; ++n )
        {
                            ...
                            elements[n] ) != d_mesh.elementsEnd() )
            {
                for ( int d = 0; d < 9; ++d )
                {
                    *( evaluated_data.begin() + n + d * num_elements ) =
                        d_comm->getRank() + 1.0 + d;
                }
            }
            else
            {
                for ( int d = 0; d < 9; ++d )
                {
                    *( evaluated_data.begin() + n + d * num_elements ) = 0.0;
                }
            }
        }
...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<GlobalOrdinal> tri_handles( num_elements );
    Teuchos::Array<GlobalOrdinal> tri_connectivity( 3 * num_elements );
    ...
    for ( int j = 0; j < ( edge_length - 1 ); ++j )
    {
        for ( int i = 0; i < ( edge_length - 1 ); ++i )
        {
            ...
            tri_connectivity[elem_idx] = vertex_handles[vertex_idx];
    ...
    Teuchos::Array<int> permutation_list( 3 );
    for ( int i = 0; i < permutation_list.size(); ++i )
    {
        permutation_list[i] = i;
    }

    return Teuchos::rcp( new MyMesh( vertex_handles, coords, tri_handles,
                                     tri_connectivity, permutation_list ) );
}
...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_test5 )
{
    ...
    Teuchos::RCP<MeshManager<MyMesh>> source_mesh_manager =
        Teuchos::rcp( new MeshManager<MyMesh>( mesh_blocks, comm, 2 ) );
    ...
    // its source rank + 1 + current dimension as data.
    double source_rank;
    for ( int n = 0; n < num_points; ++n )
    {
        ...
        {
            TEST_FLOATING_EQUALITY( source_rank + 1 + d,
...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_expanded_test5 )
{
    using namespace DataTransferKit;
    ...
    // Check the data transfer. Each target point should have been assigned
    // its source rank + 1 + current dimension as data if it is in the mesh

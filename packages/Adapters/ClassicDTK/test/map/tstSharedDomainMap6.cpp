...
class MeshTraits<MyMesh>
{
  public:
    ...
    static inline int verticesPerElement( const MyMesh &mesh ) { return 5; }
...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<GlobalOrdinal> pyr_handles( num_elements );
    Teuchos::Array<GlobalOrdinal> pyr_connectivity( 5 * num_elements );
    ...
    Teuchos::Array<int> permutation_list( 5 );
    for ( int i = 0; i < permutation_list.size(); ++i )
    {
        permutation_list[i] = i;
    }

    return Teuchos::rcp( new MyMesh( vertex_handles, coords, pyr_handles,
...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_expanded_test6 )
{
    using namespace DataTransferKit;
...
// aren't in the mesh.
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_tiled_test6 )
{
    using namespace DataTransferKit;

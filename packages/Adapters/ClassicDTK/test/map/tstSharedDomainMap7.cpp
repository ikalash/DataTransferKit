...
class MeshTraits<MyMesh>
{
  public:
    ...
    static inline int vertexDim( const MyMesh &mesh ) { return 1; }
    ...
    static inline int verticesPerElement( const MyMesh &mesh ) { return 2; }
...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    {
        vertex_handles[i] = (GlobalOrdinal)num_vertices * my_rank + i;
        coords[i] = i + my_rank * ( edge_length - 1 );
    }

    // Make the line segments.
    int num_elements = ( edge_length - 1 );
    Teuchos::Array<GlobalOrdinal> line_handles( num_elements );
    Teuchos::Array<GlobalOrdinal> line_connectivity( 2 * num_elements );
    ...
    for ( int i = 0; i < ( edge_length - 1 ); ++i )
    {
        ...
        line_connectivity[elem_idx] = vertex_handles[vertex_idx];
    ...
    return Teuchos::rcp( new MyMesh( vertex_handles, coords, line_handles,
                                     line_connectivity, permutation_list ) );
}
...
TEUCHOS_UNIT_TEST( SharedDomainMap, shared_domain_map_test7 )
{
    ...
    Teuchos::RCP<MeshManager<MyMesh>> source_mesh_manager =
        Teuchos::rcp( new MeshManager<MyMesh>( mesh_blocks, comm, 1 ) );

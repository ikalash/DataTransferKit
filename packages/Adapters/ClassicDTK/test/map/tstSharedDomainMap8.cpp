...
class MeshTraits<MyMesh>
{
  public:
    ...
    static inline int verticesPerElement( const MyMesh &mesh ) { return 6; }
...
Teuchos::RCP<MyMesh> buildMyMesh( int my_rank, int my_size, int edge_length )
{
    ...
    Teuchos::Array<int> permutation_list( 6 );
    for ( int i = 0; i < permutation_list.size(); ++i )
    {
        permutation_list[i] = i;
    }

    return Teuchos::rcp( new MyMesh( vertex_handles, coords, wedge_handles,

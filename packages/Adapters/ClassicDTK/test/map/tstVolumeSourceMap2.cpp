...
TEUCHOS_UNIT_TEST( VolumeSourceMap, cylinder_test )
{
    ...
    Teuchos::reduceAll( *comm, Teuchos::REDUCE_SUM, 1, &num_in_cylinder,
                        &global_num_in_cylinder );

    int num_missed = volume_source_map.getMissedTargetPoints().size();
    int global_num_missed = 0;
    Teuchos::reduceAll( *comm, Teuchos::REDUCE_SUM, 1, &num_missed,
                        &global_num_missed );
...
TEUCHOS_UNIT_TEST( VolumeSourceMap, box_test )
{
    ...
    Teuchos::reduceAll( *comm, Teuchos::REDUCE_SUM, 1, &num_in_box,
                        &global_num_in_box );
